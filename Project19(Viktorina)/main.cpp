...
public:
	...
	int settings() {
		...
		while (true)
		{
			CLEAR;
			std::cout << "[+] Настройки\n\n";
			std::cout << "[1] Кол. вопросов: " << number_questions << "\n";
			std::cout << "[2] Кол. жизней: " << life << "\n";
			std::cout << "[3] Кол. подсказок: " << promt << "\n";
			std::cout << "[4] Выход в меню\n\n";
			std::cout << "[>] Введите пункт меню: ";
			char choose;
			std::cin >> choose;
			switch (choose) {
			case '1': {
				char choose;
				std::cout << "[>] Введите кол. вопросов ([1]5, [2]10, [3]15): ";
				std::cin >> choose;
				if (choose == '1')
					number_questions = 5;
				else if (choose == '2')
			...
			case'2':
				...
				break;
			case'3':
				std::cout << "[>] Введите кол. подсказок: ";
				std::cin >> promt;
				if (promt > 10)
				{
					promt = 3;
	...
	void showFifty(int M, int i) {
		...
		promt--;
		gotoxy(0, 8);
		std::cout << "["; setHearthAndSmileSkin(3); std::cout << "]" << "Жизни: " << life;
		std::cout << "\t["; setHearthAndSmileSkin(2); std::cout << "]" << "Очки: " << point;
		std::cout << "\t[" << char(63) << "]" << "Подсказки(50/50): " << promt;
		gotoxy(0, 0);
		if (M == 0)
		{
...
private:
	...
	std::string questions[30] = {
		"Когда был создан C++?",
		"В каком году был основан колледж связи №54?",
		"Имя преподавателя ?",
		"Название предмета?",
		"Зимой и летом одним цветом?",
		"Что идёт перед числом 47?",
		"Долг платежём...?",
		"Кто завёл Польско-Литовское войско в болото?",
		"Какого цвета стороны у ракетки для настольного тенниса?",
		"Какое число на кроссовках Султана?",
		"2+2*2?",
		"В каком году был выпущен steam",
		"Российский мультфильм, удостоенный «Оскара», — это…?",
		"Кто из знаменитых художников за жизнь продал всего одну картину? ",
		"Друг Цезаря который участвовал в его убийстве?",//
		...
		"На флаге какой российской области изображен верблюд?",
		"Назовите самый известный балет П.И.Чайковского, посвященный птицам?",
		"Какая валюта Дании?",
		"Что означает «www» в браузере веб-сайтов?",
		"Из какого города приехали The Beatles?",
		"О каком певце говорили, что он «Король поп-музыки»?",
		"Из какой страны родом Джастин Бибер?",
		...
		"Из какого вещества состоят ногти?",
	...
	std::string answer[30][4] = {
		{"1987","1983","1995","1990 "},
		{"2003","2001","2005","2000"},
		{"Алексей","Иван","Дмитрий","Пётр"},
		{"1.04 ИСП","2.28 ИСП","БЖ","Философия"},
		{"Рыба","Ёлка","Бордюр","Аквамарин"},
		{"M4","AK","Дождь","Зима"},
		{"Красен","Должен","Обязан","Яндекс"},
		{"Сусанин","Кутузов","Лобзиков","Гитинамагомедов "},
		{"Красный и черный","Жёлтый и синий","Зелёный и красный","Лиловый и пурпурный"},
		{"208","116","400","300"},
		{"8","12","6","5"},
		{"2000","2003","1999","2013"},
		{"Ну,погоди","Старик и море","Винни пух","Простаквашино"},
		{"Репин","Моне","Мане","Ван гог"},
		{"Лонгслив","Худи","Архимед","Брут"},
		{"На трусость","На пьянство","На наркотики","На детей"},
		{"Комета","Восток","Штопор","Ракета"},
		{"Рождаемость","Космическая пыль","Программисты","Старики "},
		{"Челябинской","Московской","Воронежской ","Ростовской"},
		{"Утиный пруд","Море чаек","Лебединое озеро","Голубиная лужа "},
		{"Крона","Рубль","Евро","Доллар"},
		{"Вход на сайт","Имя бога","Всемирная паутина","Локальная паутина"},
		{"Ливерпуль","Стамбул","Москва","Лондон"},
		{"Майкл Бэй","Майкл Джордан","Майкл Джексон","Дима Билан"},
		{"Канада","США","Франция","Англия"},
		{"Пепси","Кола","Доктор пеппер","Фанта"},
		{"Аглет","Чехол","Строка","Насадка"},
		{"Race","Marathon","Smile","Sprint"},
		{"Торнадо","Землетрясения","Наводнения","Ураган"},
		{"Серотонин","Эпидермис","Меланин","Кератин"}
	};

	void showSelectAnswer() {
		std::cout << "[>] Выберите ответ: ";
	}
	...
	void showResult() {
		CLEAR;
		std::cout << "\n\t\tВаш результат: " << point << " / " << number_questions << "\n\n";
		STOP;
	}
	...
	//ahahhahahahha da konechno eto otdelniy metod pochemu бbI i net)))
	...
	void rules() {
		CLEAR;
		std::cout << "Правила викторины очень просты!\n";
		std::cout << "Отвечайте на вопросы и зарабатывайте очки.\n";
		std::cout << "Вы можете использовать подсказку нажав на кнопку \'h\'.\nБудьте осторожны если у вас нет подсказок, это будет засчитано как неправильный ответ!\n";
		std::cout << "Удачи!:)\n\n";
		STOP;
	}
};
